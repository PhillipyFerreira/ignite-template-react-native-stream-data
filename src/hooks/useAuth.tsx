...
function AuthProvider({ children }: AuthProviderData) {
  ...
  async function signIn() {
    try {
      ...
      if (
        response.type === "success" && response.params.error !== "access_denied"
      ){
        ...
        setUserToken(response.params.access_toke);
      }
    } catch (error) {
      // throw an error
      throw new Error(`Erro ${error} durante a autenticação com a Twitch`);
    } finally {
